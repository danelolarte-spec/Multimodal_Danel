...
- **Módulos aún no conectados a la API**: Infracciones, Pólizas/Reclamaciones, Convenios, Leasing, Caja menor, Renovaciones mensuales, Solicitudes de afiliación y Operaciones (Contratos/Servicios) siguen usando datos de ejemplo en memoria del navegador — la API para todos ellos ya existe (ver `docs/BACKEND_DESIGN.md`), falta repetir en cada uno el mismo patrón de conexión ya aplicado a Vehículos, Conductores, Cartera y Extractos (FUEC).
