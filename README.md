...
**Tarifario por cliente y "generar nueva ruta"**: cada contrato (`ContratoFormModal`) tiene su propio tarifario (`contrato.productos`) con tipo de servicio, ruta (origen/destino opcional), tarifa a cobrar al cliente y pago a afiliado/convenio. Al crear un servicio (`ServicioFormModal`), seleccionar el contrato/cliente precarga como opciones solo las rutas y tipos de servicio ya tarifados para él (con su tarifa); si el servicio que se necesita no está contemplado, la opción "+ Generar nueva ruta / servicio no tarifado" permite definirlo con su tarifa a cobrar y a pagar, y guardarlo en el tarifario del contrato para que quede disponible en los próximos servicios de ese cliente. Es un tarifario independiente del que usa el módulo Comercial (`tarifario_items`, con backend real) — ver `docs/BACKEND_DESIGN.md` §9.5.

### Módulo "Comercial" (`ComercialApp`)

Reutiliza los clientes/contratos de Extractos (§8 de `docs/BACKEND_DESIGN.md`) en vez de duplicar infraestructura. Alta de cliente corporativo con: contrato firmado (adjunto de una vez), tarifario aplicable en modo tabla editable (tipo de servicio, tipo de vehículo, origen/destino opcionales, valor del servicio en pesos, pago a afiliados/convenios), ruta del contrato elegida entre las rutas ya tarifadas en esa misma tabla (o "+ Nueva ruta" para definirla a mano), y envío automático de una solicitud de verificación a Trámites (el contrato entra directo en `PENDIENTE_VALIDACION`) para que se revise y apruebe antes de poder generar extractos a nombre de ese cliente — como el motor de validación exige que el extracto coincida con la ruta del contrato, los extractos quedan así restringidos a las rutas/servicios tarifados al cliente. Detalle completo en `docs/BACKEND_DESIGN.md` §9.
