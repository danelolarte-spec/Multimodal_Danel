...
**Clientes Comercial** (nuevo ítem del menú): lista los clientes dados de alta en Comercial y ya aprobados por Trámites, con badge de si Operaciones ya diseñó su formulario de servicios. El diseño son campos adicionales (además de los que todo servicio ya trae por defecto: fecha, hora, origen/destino, vehículo, conductor, pasajeros, valor) — el tarifario en sí no se edita ahí, lo administra Comercial.

### Módulo "Comercial" (`ComercialApp`)

Reutiliza los clientes/contratos de Extractos (§8 de `docs/BACKEND_DESIGN.md`) en vez de duplicar infraestructura. Alta de cliente corporativo con: contrato firmado (adjunto de una vez), tarifario aplicable en modo tabla editable y sin límite de filas (tipo de servicio, tipo de vehículo, descripción, origen/destino opcionales, valor del servicio en pesos, pago a afiliados/convenios), y envío automático de una solicitud de verificación a Trámites (el contrato entra directo en `PENDIENTE_VALIDACION`) para que se revise y apruebe antes de poder generar extractos a nombre de ese cliente. El contrato ya no pide ni limita una ruta única — el tarifario del cliente es la única fuente de rutas/servicios, tanto para los extractos (Trámites) como para los servicios (Operaciones), y es editable en cualquier momento desde el detalle del cliente en Comercial o desde el detalle del contrato en Trámites. Detalle completo en `docs/BACKEND_DESIGN.md` §9.
